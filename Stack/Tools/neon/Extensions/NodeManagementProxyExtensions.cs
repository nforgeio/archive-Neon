...
namespace NeonCluster
{
    ...
    public static class NodeManagentProxyExtension
    {
        ...
        private static void SetClusterVariables(PreprocessReader preprocessReader, ClusterDefinition clusterDefinition, NodeDefinition nodeDefinition)
        {
            ...
            {
                // systemd doesn't use a shell, so multi-line environment variables won't work and I
                // also couldn't get a drop-in systemd file to work with environment variables.  Instead,
                // I'm going to drop trying to use environment variables in this case and simply hardcode
                // the node options directly into the service command line.
                ...
                sbDockerOptions.AppendWithSeparator($"--log-level=info");

                if (!string.IsNullOrWhiteSpace(clusterDefinition.Docker.LogOptions))
                {
                    sbDockerOptions.AppendWithSeparator(clusterDefinition.Docker.LogOptions);
                }
                ...
                if (clusterDefinition.Docker.RegistryCache)
                {
                    foreach (var manager in clusterDefinition.SortedManagers)
                    {
                        sbDockerOptions.AppendWithSeparator($"--registry-mirror https://{manager.Name}.{NeonHosts.RegistryCache}:{NeonHostPorts.RegistryCache}");
                    }
                }

                sbDockerOptions.AppendWithSeparator($"--registry-mirror {clusterDefinition.Docker.Registry}");
            }
            else
            {
            ...
            switch (Program.OSProperties.StorageDriver)
            {
                case DockerStorageDrivers.Aufs:
                    ...
                    sbDockerOptions.Append(" --storage-driver aufs");
                    break;

                case DockerStorageDrivers.Overlay:

                    sbDockerOptions.Append(" --storage-driver overlay");
                    break;

                case DockerStorageDrivers.Overlay2:

                    sbDockerOptions.Append(" --storage-driver overlay2");
                    break;
