...
namespace NeonCluster
{
    ...
    public class SetupController
    {
        ...
        /// </param>
        /// <param name="quiet">Optionally specifies that the step is not to be reported in the progress.</param>
        public void AddWaitUntilOnlineStep(string stepSummary = "connect", string status = null, Func<NodeProxy<NodeDefinition>, bool> nodePredicate = null, bool quiet = false)
        {
        ...
        public bool Run(bool leaveNodesConnected = false)
        {
            ...
            finally
            {
                ...
                {
                    // Dispose all of the nodes so they'll close any open connections.
        ...
        private void DisplayStatus(HashSet<string> stepNodeNamesSet = null)
        {
            ...
            {
                if (nodes.Exists(n => n.Metadata.Manager))
                {
                    sbDisplay.AppendLine();
                    sbDisplay.AppendLine();
                ...
                if (nodes.Exists(n => n.Metadata.Worker))
                {
                    ...
                    foreach (var node in nodes.Where(n => n.Metadata.Worker))
                    {
                        sbDisplay.AppendLine($"    {node.Name}{new string(' ', maxNameWidth - node.Name.Length)}   {GetStatus(stepNodeNamesSet, node)}");
                    }
                }
