...
# or with a non-zero exit code if the step was previously started
...
function startsetup
{
    ...
    if [ -f "${NEON_STATE_FOLDER}/pending$1" ] ; then
        echo "*** ERROR: [$1] already started and did not complete." >&2
        exit 1
    fi
    ...
    touch "${NEON_STATE_FOLDER}/pending-$1"
}
...
function endsetup
{
    ...
    mkdir -p ${NEON_STATE_FOLDER}
    rm -f "${NEON_STATE_FOLDER}/pending-$1"
