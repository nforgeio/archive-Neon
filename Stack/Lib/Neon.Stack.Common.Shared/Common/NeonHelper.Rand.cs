...
namespace Neon.Stack.Common
{
    public static partial class NeonHelper
    {
        ...
        public static byte[] RandBytes(int count)
        {
            ...
            // $todo(jeff.lill):
            //
            // The bytes returned won't be cryptographically random for the PCL library
            // because the PCL profile doesn't include the Crypto APIs.  This will likely
            // change with .NET Core.  I'll revisit this then.
            //
            // In the meantime, I'm going to return a pseudo randome sequence of bytes.

            for (int i = 0; i < count; i++)
            {
                var v = RandInt();
                
                bytes[i] = (byte)(v ^ (v >> 8) ^ (v >> 16) ^ (v >> 24));
            }
#else
            if (randCrypto == null)
            {
