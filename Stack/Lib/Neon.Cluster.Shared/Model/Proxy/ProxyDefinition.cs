        ...
        public ProxyValidationContext Validate(Dictionary<string, TlsCertificate> certificates)
        {
            ...
            foreach (var route in Routes.Values.Where(r => r.Mode == ProxyMode.Http).OrderBy(r => r.Name.ToLowerInvariant()))
            {
                ...
                foreach (var frontend in httpRoute.Frontends)
                {
                    ...
                    if (!httpPortToTls.TryGetValue(frontend.Port, out isTls))
                    {
                        isTls = frontend.Tls;
                        httpPortToTls.Add(frontend.Port, isTls);
                    }
                    ...
                    if (isTls != frontend.Tls)
                    {
                        if (frontend.Tls)
                        {
                            context.Error($"HTTP route [{httpRoute.Name}] has a TLS frontend on port [{frontend.Port}] that conflicts with non-TLS frontends on this port.");
                        }
                        else
                        {
                            context.Error($"HTTP route [{httpRoute.Name}] has a non-TLS frontend on port [{frontend.Port}] that conflicts with TLS frontends on this port.");
                        }
                    }
                }
            }
